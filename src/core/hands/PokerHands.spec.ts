...
describe("Poker Hands Manager", () => {
  describe("|-> Main class", () => {
    ...
    describe("|-> Validation", () => {
      ...
      test.each(invalidHandStringCases)(
        "should throw Error if hand has %s",
        (_, invalidHandString, error) => {
    ...
    describe("|-> Ranking", () => {
      describe("|-> Simple Cases", () => {
        it("should rank two high cards", () => {
          ...
          const [first, second] = handsManager.getRankedHandsScores(
            smallCardHandString,
            highCardHandString
          );
      ...
      describe("|-> Draws", () => {
        ...
        it("should rank two equal pairs according kicker value", () => {
          ...
          let [first, second] = handsManager.getRankedHandsScores(
            smallKickerPairHandString,
            highKickerPairHandString
          );
          expect(first.value).toEqual(HandValue.Pair);
          ...
          [first, second] = handsManager.getRankedHandsScores(
            smallKickerPairHandString,
            highKickerPairHandString
          );
          expect(first.value).toEqual(HandValue.Pair);
    ...
    describe("|-> Output", () => {
      const outputTestCases: [string, string[]][] = [
        [
          "John wins. - with High Card: AH",
          ["2H 3D 5S 9C KD", "2C 3H 4S 8C AH"],
        ],
        [
          "Bob wins. - with Full House: 4H, 4C, 4S, 2D, 2H",
          ["2H 4S 4C 2D 4H", "2S 8S AS QS 3S"],
        ],
        [
          "Bob wins. - with High Card: KD",
          ["2H 3D 5S 9C KD", "2C 3H 4S 8C KH"],
        ],
        ["Tie", ["2H 3D 5S 9C KD", "2D 3H 5C 9S KH"]],
      ];
      test.each(outputTestCases)(
